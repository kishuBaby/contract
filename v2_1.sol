...
contract kishuBaby is Context, IERC20, Ownable {
    ...
    constructor (string memory _NAME, string memory _SYMBOL, uint256 _DECIMALS, uint256 _supply, uint256 _mktFee, uint256 _txFeeBuy,uint256 _lpFeeBuy,uint256 _txFeeSell,uint256 _lpFeeSell,uint256 _MAXAMOUNT_BUY,uint256 _MAXAMOUNT_SELL,uint256 SELLMAXAMOUNT,address routerAddress,address tokenOwner) public {
        _name = _NAME;
        _symbol = _SYMBOL;
        _decimals = _DECIMALS;
    ...
    function setTaxFeePercentBuy(uint256 marketingFee) external onlyOwner() {
    ...
    function _transfer(
        ...
        if(!(isExcludedFromAntiWhale(from) || isExcludedFromAntiWhale(to))) { // white list of antiwhale excluded from maxAmount
            if(to == uniswapV2Pair){
                ...
                require(amount <= _maxTxAmountBuy, "Transfer amount exceeds the maxTxAmount buy or transfer.");
